...
APP_DISPLAY_NAME = os.getenv("APP_DISPLAY_NAME", "昆山市尚为人力资源配置服务有限公司投标助手")
EXE_NAME = os.getenv("EXE_NAME", "ks-shangwei-bid-assistant")


def run_command(cmd, cwd=None):
    """运行命令"""
    if isinstance(cmd, (list, tuple)):
        cmd_display = " ".join(str(item) for item in cmd)
        shell_mode = False
    else:
        cmd_display = str(cmd)
        shell_mode = True
...
def build_exe():
    ...
    # 构建exe - 通过 python -m PyInstaller 调用，避免系统 PATH 导致失败
    pyinstaller_cmd = [
        ...
        "--hidden-import=aiohttp", "--hidden-import=aiohttp.client", "--hidden-import=aiohttp.connector",
        "--collect-all", "aiohttp",
        "--hidden-import=openai", "--hidden-import=docx", "--hidden-import=docx.oxml",
...
def main():
    ...
    print("\n" + "=" * 50)
    print("构建完成！")
    print(f"exe文件位于: dist/{EXE_NAME}.exe")
    print("=" * 50)
